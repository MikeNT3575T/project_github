...
int __cdecl wmain(int argc, _In_reads_(argc) WCHAR** argv)
{
	...
	while (true) {
		...
		int   nLen = strlen(buf) + 1;
		int   nwLen = MultiByteToWideChar(CP_ACP, 0, buf, nLen, NULL, 0);
		WCHAR  buf2[512];
		MultiByteToWideChar(CP_ACP, 0, buf, nLen, buf2, nwLen);
		if (buf != NULL)
		{
			RETCODE     RetCode;
			...
			string str1 = buf;
			wstring wstr1;
			string str2 = "' INSERT INTO TEST VALUES (@BUF);";
			wstring wstr2;
			wstr0.assign(str0.begin(), str0.end());
			wstr0.append(str1.begin(), str1.end());
			...
			wstr3 += wstr1;
			wstr3 += wstr2;*/
			const WCHAR* wszInput = wstr0.c_str();
			//-----------------------------------------------------
