...
int __cdecl wmain(int argc, _In_reads_(argc) WCHAR** argv)
{
	...
	while (true) {
		...
		if (ltm->tm_min == 0) {
			...
			wstrtime += L") begin UPDATE test2 SET Count = (SELECT COUNT(STA_Mac) FROM syslogtest4) FROM test2 INNER JOIN syslogtest4 ON test2.Time = syslogtest4.Time TRUNCATE TABLE syslogtest4 end";
