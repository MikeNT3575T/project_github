...
void syslogtest(wstring& syslog);
/******************************************/
/* Forward references                     */
/******************************************/
	...
	while (true) {
		...
		if (buf != NULL)
		{
			...
			*/
			...
			syslogtest(wstr1);
			wstring wstr2 = L"INSERT INTO syslogtest VALUES (";
			//wstring wstr2;
			wstr2.append(wstr1.begin(), wstr1.end());
			wcout << wstr2;
			/*
			...
			*/
			...
			wstr3 += wstr1;
			wstr3 += wstr2;*/
			const WCHAR* wszInput = wstr2.c_str();
			//-----------------------------------------------------
			//WCHAR  wszInput2[SQL_QUERY_SIZE] = L"SELECT * FROM 新文字文件";
			/**/
			// Execute the query 
			if (!(*wszInput))
			{
...
void syslogtest(wstring& syslog) {
	...
	int k = 1;
	for (int i = 0;i < 6;++i) {
		for (;j < syslog.length();++j) {
			switch (count) {
			case 1:
				if (syslog[j] == '>') {
					j++;
					copy = '\'';
					copy.append(syslog, j + 16, 5);
					copy.append(syslog, j, 6);
					copy += '\'';
					j += 7;
					copy += ',';
					count++;
