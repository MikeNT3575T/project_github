...
int __cdecl wmain(int argc, _In_reads_(argc) WCHAR** argv)
{
	...
	while (true) {
		...
		if (ltm->tm_min == 0 && ltm->tm_sec == 0) {
			...
			wstring wstrtime = L"UPDATE syslogtest6 SET Count = (SELECT COUNT(STA_Mac) FROM syslogtest4) FROM syslogtest6 INNER JOIN syslogtest4 ON syslogtest6.Time = syslogtest4.Time TRUNCATE TABLE syslogtest4 IF (SELECT COUNT(STA_Mac) FROM syslogtest4)=0 INSERT INTO syslogtest4 SELECT syslogtest2.Time, syslogtest2.STA_Mac from syslogtest2";
			wstrtime += L" UPDATE syslogtest7 SET Count = (SELECT COUNT(AP_Mac) FROM syslogtest5) FROM syslogtest7 INNER JOIN syslogtest5 ON syslogtest7.Time = syslogtest5.Time TRUNCATE TABLE syslogtest5";
		...
		else if (proreq != 0)
		{
			...
			for (index = rssi.begin();index != rssi.end();++index)
				sqsum += pow((*index), 2);
			float stand = sqrt((sqsum / num) - pow(mean, 2));
			int sigthr = round(mean + stand);
			cout << round(mean) << " " << sigthr << '\n';
			wstring sigthr2 = to_wstring(sigthr);
			wstring wstrtable9 = L" UPDATE syslogtest3 SET RSSI = '-";
			wstrtable9 += sigthr2;
			wstrtable9 += '\'';
			wstring wstrtable8 = L" IF NOT EXISTS(SELECT * FROM syslogtest5 WHERE AP_Mac = '";
			wstrtable8 += apmac;
			wstrtable8 += L"') INSERT INTO syslogtest5 VALUES (";
			wstrtable8.append(tmnow.begin(), tmnow.end());
			wstrtable8 += L", \'";
			wstrtable8 += apmac;
			wstrtable8 += L"\')";
			wstrtable8 += L" ELSE UPDATE syslogtest5 SET Time = ";
			wstrtable8.append(tmnow.begin(), tmnow.end());
			wstrtable9.append(wstrtable8.begin(), wstrtable8.end());
			const WCHAR* wszInput3 = wstrtable9.c_str();
			//========================================================================================
			// Execute the query 
			if (!(*wszInput3))
			{
